...
$(document).ready(function(){
 var status = '';
$.post(HTTP_ROOT+"easycases/task_running",
{ 
	time_stamp: new Date()
},function(data) {
         	 console.log(data);
         	 var x = JSON.parse(data).tot_dur;
         	 
         	 var op = x.split(":");        	
         	 
	if(JSON.parse(data).status == 'start'){
		console.log('hello');
		$('#demo').show(); 
		...
		var h1 = document.getElementsByTagName('h1')[0],
		    ...
		    seconds = op['2'], minutes = op['1'], hours = op['0'],
		    t;
		...
		function add() {
		   ...
		   h1.textContent = (hours ? (hours > 9 ? hours : "" + hours) : "00") + ":" + (minutes ? (minutes > 9 ? minutes : "" + minutes) : "00") + ":" + (seconds > 9 ? seconds : "0" + seconds);
		    timer();
		}
...
function startCase(id, no, dtlsid) {
				  ...
				  var conf = confirm("Are you sure you want to Start the task #"+no+" ?");
					if(conf == false) {
						...
						var status = '';
  $.get(HTTP_ROOT+"easycases/check_task_status",
         { 
         	...
         	if(JSON.parse(data).status == 'start'){
				...
				$.post(HTTP_ROOT+"easycases/data_req",
				         { 
				         	time_stamp: new Date(), brow_detail: navigator.userAgent, task_id: id 
				         },
				);
				    $('#demo').show();            
				    var h1 = document.getElementsByTagName('h1')[0],
         		  ...
         		  alert('You have already working on a task');
         		  return false;
         	    }
...
function caseResolve(id, no, dtlsid) {
	...
	if(conf == false) {
		...
		return false;
		 ...
		 $.post(HTTP_ROOT+"easycases/task_update",
         { 
         	end_stamp: new Date(), task_id: id 
         }, function(data){
         	console.log(data);
            clearInterval(data.tot_dur);
...
function Pause_timer(id, no, dtlsid) {
var conf = confirm("Are you sure you want to Pause the task #"+no+" ?");
if(conf == false) {
	$('#casePause').val("");			//enableButtons(); not impl
		return false;
} 
else {	
	   $.ajax({
	       "url" : HTTP_ROOT+"easycases/pause_timer",
	       "method" : 'post',
	       "data" : {'time_stamp' : new Date() , 'task_id': id},
...
function Resume_timer(id, no, dtlsid) {
	var conf = confirm("Are you sure you want to Resume the task #"+no+" ?");
	if(conf == false) {
	$('#caseResume').val("");	
	return false;
	} else {
	    $.ajax({
	       "url" : HTTP_ROOT+"easycases/resume_timer",
	       "method" : 'post',
	       "data" : {'time_stamp' : new Date() , 'task_id': id , 'brow_detail': navigator.userAgent},
	       "success" : function(data){
	       	 $(document).ready(function() {
	       	 	console.log("shkkk");
	       	     	$('#demo').show();
                	$('#dem').hide();
		       		console.log('data');						
		       		console.log(JSON.parse(data).tot_dur);						
		      		var x = JSON.parse(data).tot_dur;
		      		var op = x.split(":"); 
		       		var h1 = document.getElementsByTagName('h1')[0],
				    start = document.getElementById('start'),
