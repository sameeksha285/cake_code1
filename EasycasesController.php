...
#### DELETING CASES CREATED BY LOGGED IN USER OR LOGGED IN AS ADMIN USER #######
   ...
   function ajaxpostcase($oauth_arg = NULL) {
        ...
        if (isset($oauth_arg) && !empty($oauth_arg)) {
            ...
            // $CS_estime = $this->params->data['CS_estime'];
...
function data_req(){
       $this->loadModel('Timelog');
        ...
        //To get Internet IP
        // $ip_data = file_get_contents('http://ipinfo.io');
        // echo $ip_data;
        
        //echo json_encode($this->params->data['brow_detail']); 
        $time_stamp = $this->params->data['time_stamp'];
        $task_id = json_encode($this->params->data['task_id']);
        $select_data = $this->Easycase->query("SELECT project_id  FROM easycases WHERE user_id =".SES_ID."  AND id=".$task_id." " );

        $this->Easycase->query("INSERT INTO time_log SET time_stamp='" . $this->params->data['time_stamp'] . "', user_id='" . SES_ID . "', browser_id='" . $this->params->data['brow_detail'] . "' , ip='" . $ip_add . "', company_id='" . 1 . "',status='" . 1 . "', task_id='" .  $this->params->data['task_id'] . "' , project_id='" . $select_data [0]['easycases']['project_id'] . "'");
       
        $allHelpData = $this->Easycase->query("SELECT time_stamp FROM time_log where task_id = $task_id");
        $estimate_time = $this->Easycase->query("SELECT estimated_hours FROM easycases where user_id = '".SES_ID."' AND id =".$task_id);
       
        echo json_encode($estimate_time[0]['easycases']['estimated_hours']);
        // if( == $estimate_time[0]['easycases']['estimated_hours'])
    ...
    function task_running(){
        ...
        $no_entries = $this->Easycase->query("SELECT count(task_id) as task_id FROM time_log where user_id = '".SES_ID."' AND status = 1");
        $current_time_stamp = $this->params->data['time_stamp'];
    if($no_entries[0][0]['task_id'] > 1){

        $task_id2 = $this->Easycase->query("SELECT task_id FROM time_log where user_id = '".SES_ID."' AND end_time_stamp = '' AND status = 1 ORDER BY id DESC ");
        $task_id = $task_id2[0]['time_log']['task_id'];
        //echo $task_id ;
        $check_running = $this->Easycase->query("SELECT * FROM time_log WHERE user_id = '".SES_ID."'  AND task_id = $task_id AND status = 0 ORDER By id DESC ");
        //echo json_encode($check_running);
        $gettime_stamp = $this->Easycase->query("SELECT * FROM time_log where user_id = '".SES_ID."' AND end_time_stamp = '' AND task_id = $task_id ORDER BY id DESC ");
        $time_stamp = $gettime_stamp[0]['time_log']['time_stamp'];
        //echo $time_stamp;exit();
        $gettime_pause_stamp = $this->Easycase->query("SELECT Timer_duration FROM time_log where id < ( SELECT MAX( id )
                 FROM time_log ) AND user_id = '".SES_ID."' AND end_time_stamp = '' AND status = 1
           AND task_id = $task_id  ORDER BY id DESC");
        $get_prev_dur =  $gettime_pause_stamp[0]['time_log']['Timer_duration'];
        // $time_log = $this->Easycase->query("SELECT * FROM time_log where user_id = '".SES_ID."' AND end_time_stamp = '' AND status = 1");
        $gettime_pause = $this->Easycase->query("SELECT status FROM time_log where user_id = '".SES_ID."' AND end_time_stamp = '' AND task_id = $task_id ORDER BY id DESC");

        $status = $gettime_pause[0]['time_log']['status'];

        $d1 = explode(" ", $time_stamp);
        $d2 = explode(" ",   $current_time_stamp); 
        
        $dteStart = new DateTime($d1[4]); 
        $dteEnd   = new DateTime($d2[4]);

        $dteDiff  = $dteStart->diff($dteEnd); 
        $tot_dur1 = $dteDiff->format("%H:%I:%S");
        //echo json_encode($tot_dur1);

        $secs = strtotime($tot_dur1)-strtotime("00:00:00");
        $result = date("H:i:s",strtotime($get_prev_dur)+$secs);
        
        // $task_id =  $this->Easycase->query("SELECT task_id FROM time_log WHERE user_id = '".SES_ID."' AND status = 2");
        // $task = $task_id[0]['time_log']['task_id'];
        ...
        if($check_running){
            ...
            echo json_encode($data);exit();
               ...
               $data = ['status' => 'start' , 'estimate_time' =>$estimate_in_time1,'time_left_final'=>$time_left_final, 'tot_dur' =>  $result , 'pause'=> $status , 'task_id' => $task_id , 'time_log' => $time_log[0]['time_log']['Timer_duration']];
            echo json_encode($data);exit();
       ...
       //echo 'hii';
        $gettime_stamp = $this->Easycase->query("SELECT * FROM time_log where user_id = '".SES_ID."' AND end_time_stamp = ''");
        $gettime_pause_stamp = $this->Easycase->query("SELECT * FROM time_log where user_id = '".SES_ID."' AND end_time_stamp = ''  ");

        $time_log = $this->Easycase->query("SELECT * FROM time_log where user_id = '".SES_ID."' AND end_time_stamp = '' AND status = 1");
        
        $d1 = explode(" ", $current_time_stamp); 
        $d2 = explode(" ", $gettime_stamp[0]['time_log']['time_stamp']);
        ...
        $task_id2 = $this->Easycase->query("SELECT task_id FROM time_log where user_id = '".SES_ID."' AND end_time_stamp = '' AND status = 1 OR status = 2 ORDER BY id DESC ");
        $task_id = $task_id2[0]['time_log']['task_id'];
        //echo $task_id ;exit();
        $gettime_pause = $this->Easycase->query("SELECT status FROM time_log where user_id = '".SES_ID."' AND end_time_stamp = '' AND task_id = $task_id ORDER BY id DESC");
        //echo json_encode($gettime_pause);exit();
        $status = $gettime_pause[0]['time_log']['status'];
        //echo $status;exit();
        $check_running = $this->Easycase->query("SELECT * FROM time_log WHERE user_id = '".SES_ID."' AND status = 0 AND  task_id = $task_id ORDER By id DESC ");
         //echo json_encode($check_running);exit();
        if($check_running){
            ...
            echo json_encode($data);exit();
             ...
             $data = ['status' => 'start','estimate_time' =>$estimate_in_time1,'time_left_final'=>$time_left_final, 'tot_dur' => $tot_dur1,'pause'=> $status , 'task_id' =>$task,'time_log' => $time_log[0]['time_log']['Timer_duration']];
            echo json_encode($data);exit();
    ...
    function task_update(){
        ...
        $end_task = $this->params->data['end_stamp'];
        $task_id = $this->params->data['task_id'];
        $gettime_stamp = $this->Easycase->query("SELECT time_stamp FROM time_log where task_id = '".$task_id."' AND status= 1 OR status = 2 ORDER BY id DESC ");
        $no_entries = $this->Easycase->query("SELECT count(task_id) as task_id FROM time_log where user_id = '".SES_ID."' AND status = 1 AND task_id = $task_id ");
        $get_status = $this->Easycase->query("SELECT status FROM time_log where user_id = '".SES_ID."' AND task_id = task_id AND status = 2 ORDER BY id DESC");
        $gettime_pause = $this->Easycase->query("SELECT Timer_duration FROM time_log where  user_id = '".SES_ID."' AND task_id = '".$task_id."' AND status = 2");
        //echo  json_encode($gettime_pause) ;exit();
        $get_data = $gettime_stamp[0]['time_log']['time_stamp'];
        echo $get_data;
        $pause_time = $gettime_pause[0]['time_log']['Timer_duration'];
        $gettime_pause_stamp = $this->Easycase->query("SELECT Timer_duration FROM time_log where id < ( SELECT MAX( id )
                 FROM time_log ) AND user_id = '".SES_ID."' AND end_time_stamp = '' AND status = 1
           AND task_id = $task_id  ORDER BY id DESC");
        
        //echo $get_prev_dur; echo "hiii";
      
        $d1 = explode(" ",  $get_data); 
        $d2 = explode(" ",$end_task);
        $d3 = explode("", $pause_time);
       
        $dteStart = new DateTime($d1[4]); 
        $dteEnd   = new DateTime($d2[4]);
        $dtefinish  = new DateTime($d3[4]);
        $dteDiff  = $dteStart->diff($dteEnd); 
        $dteDiff1 = $dteEnd->diff($dtefinish); 
        $tot_dur = $dteDiff->format("%H:%I:%S");

        echo $tot_dur ; echo '<br>';
        $get_prev_dur =  $gettime_pause_stamp[0]['time_log']['Timer_duration'];
        echo $get_prev_dur;echo '<br>';
        $secs = strtotime($tot_dur)-strtotime("00:00:00");
        $result = date("H:i:s",strtotime($get_prev_dur)+$secs);
        echo $result ;
        //$final = $dteDiff1->format("%H:%I:%S");
        
        //$tot_time = $tot_dur + $final;
        //$tot_time_dur = date("H:i:s",$tot_time);
        
        if($no_entries[0][0]['task_id'] > 1){

             if($get_status){
               //echo "pause case";
              $this->Easycase->query("UPDATE time_log SET end_time_stamp= '".$end_task."', status= 0  WHERE task_id= '".$task_id."' AND user_id = '".SES_ID."' AND end_time_stamp = '' AND status = 2  ");
               

             }else{
                  //echo "pause and resume case";
                 $this->Easycase->query("UPDATE time_log SET end_time_stamp= '".$end_task."', Timer_duration = '".$result."', status= 0  WHERE task_id= '".$task_id."' AND user_id = '".SES_ID."' AND pause_timer = '' AND  end_time_stamp = '' AND Timer_duration = ''  "); 
                  $data = ['tot_dur' => $result];
                  return $data;
        ...
        //echo "No case";
        $this->Easycase->query("UPDATE time_log SET end_time_stamp= '".$end_task."', status= 0 ,Timer_duration = '".$tot_dur."'  WHERE task_id= '".$task_id."' AND user_id = '".SES_ID."' AND status = 1 ");

           $data = ['tot_dur' => $tot_dur];
                return $data;
          }      
        }
    ...
    function check_task_status(){
        $id = SES_ID ;
        //$getData = $this->Easycase->query("SELECT task_id FROM time_log where user_id = ".SES_ID);
        //echo json_encode($getData);exit();
        $gettime_stamp = $this->Easycase->query("SELECT count(end_time_stamp) as end_time_stamp FROM time_log where user_id = '".SES_ID."' AND end_time_stamp = '' AND pause_timer= '' AND Timer_duration = '' AND status = 1 ORDER BY id DESC ");
        //echo json_encode($gettime_stamp);exit();
        
        if($gettime_stamp[0][0]['end_time_stamp'] > 0){
            $data = ['status' => 'stop'];
          echo json_encode($data);exit();
    ...
    function pause_timer(){  
        ...
        $gettime_pause_stamp = $this->Easycase->query("SELECT Timer_duration FROM time_log where id < ( SELECT MAX( id )
                 FROM time_log ) AND user_id = '".SES_ID."' AND end_time_stamp = '' AND status = 1
           AND task_id = $task_id  ORDER BY id DESC");
        //echo json_encode($gettime_pause_stamp);
        $get_prev_dur =  $gettime_pause_stamp[0]['time_log']['Timer_duration'];
        //echo $get_prev_dur; echo "hiii";
        $time_stamp = $gettime_stamp[0]['time_log']['time_stamp'];
        //echo $time_stamp; echo '<br>';
        $d1 = explode(" ", $time_stamp); 
        $d2 = explode(" ", $current_time_stamp);
        $dteStart = new DateTime($d1[4]); 
        $dteEnd   = new DateTime($d2[4]);
        ...
        if($no_entries[0][0]['task_id'] > 1){
           $this->Easycase->query("UPDATE time_log SET status= 2 , pause_timer = '".$current_time_stamp."' , Timer_duration = '".$result."'  WHERE user_id= '".SES_ID."' AND end_time_stamp = '' AND task_id = $task_id AND Timer_duration = '' ");
           
            $data = ['status' => 'start', 'tot_dur' => $result];
            echo json_encode($data);exit();

        }else{

           $this->Easycase->query("UPDATE time_log SET status= 2 , pause_timer = '".$current_time_stamp."' , Timer_duration = '".$tot_dur ."'  WHERE user_id= '".SES_ID."' AND end_time_stamp = '' AND task_id = $task_id AND Timer_duration = '' ");
           
            $data = ['status' => 'start', 'tot_dur' => $tot_dur];
            echo json_encode($data);exit();
    ...
    function all_Data(){
      ...
      $tb_data = $this->Easycase->query("SELECT easycases.title, projects.name ,time_log.time_stamp, time_log.Timer_duration , users.name  FROM projects  INNER JOIN easycases
          ON projects.id = easycases.project_id INNER JOIN  time_log ON easycases.id = time_log.task_id INNER JOIN  users ON users.id = time_log.user_id;
           ");
      
      $data_send = array();
      $data = array();

       foreach ($tb_data as $row) {
            $tb_data['title'] = $row['easycases']["title"];
            $tb_data['name'] = $row['projects']["name"];
            $tb_data['time_stamp'] = $row['time_log']["time_stamp"];
            $tb_data['Timer_duration'] = $row['time_log']["Timer_duration"];
            $tb_data['user_name'] = $row['users']["name"];
            $d1 = trim($tb_data['time_stamp'], "GMT+0530 (India Standard Time)");
            $d2 = trim($d1 , "on");
            $data_form = [ "title" =>  $tb_data['title'] , "name" =>  $tb_data['name'] , "time_stamp" => $d2 , "Timer_duration" => $tb_data['Timer_duration'] , "uname" => $tb_data['user_name']];  
             array_push($data_send, $data_form );
    ...
    function getData(){
      ...
      $tb_data = $this->Easycase->query("SELECT easycases.title, projects.name , time_log.time_stamp , time_log.Timer_duration FROM projects  INNER JOIN easycases
          ON   projects.id = easycases.project_id INNER JOIN  time_log ON easycases.id = time_log.task_id  WHERE time_log.user_id = '".SES_ID."'  ;
           ");
      ...
      $data = array();
       ...
       foreach ($tb_data as $row) {
            ...
            $d1 = trim($tb_data['time_stamp'], "GMT+0530 (India Standard Time)");
            $d2 = trim($d1 , "on");
            $data_form = [ "title" =>  $tb_data['title'] , "name" =>  $tb_data['name'] ,  "time_stamp" => $d2 , "Timer_duration" => $tb_data['Timer_duration']];  
            array_push($data_send, $data_form );
    ...
    $ip_add = $_SERVER[REMOTE_ADDR];
    $brow_detail = $this->params->data['brow_detail'];
    $current_time_stamp = $this->params->data['time_stamp'];
    $task_id = $this->params->data['task_id'];
    $prj_id = $this->Easycase->query("SELECT project_id FROM easycases where user_id = '".SES_ID."' AND id = $task_id  ");
    $task_start_frm = $this->Easycase->query("SELECT Timer_duration FROM time_log where user_id = '".SES_ID."' AND end_time_stamp = '' AND task_id = $task_id AND status = 2 ");
    $timer_duration = $task_start_frm[0]['time_log']['Timer_duration'];
   
    $this->Easycase->query("UPDATE time_log SET status= 1  WHERE user_id= '".SES_ID."' AND end_time_stamp = '' AND task_id = $task_id  AND status = 2  ");

    $this->Easycase->query("INSERT INTO time_log SET time_stamp='" . $current_time_stamp . "', user_id='" . SES_ID . "', browser_id='" . $brow_detail . "' , ip='" . $ip_add . "', company_id='" . 1 . "',status='" . 1 . "', task_id='" .  $task_id . "' , project_id='" .$prj_id[0]['easycases']['project_id'] . "'");    
    
    
       if( $task_start_frm){
          $data = ['tot_dur' => $timer_duration];
          echo json_encode($data);exit();
